...
Optional environment variables:
    BITBUCKET_PROJECT   - Bitbucket project key (if not set, searches all workspace repos)
...
Examples:
    ...
    BITBUCKET_USERNAME=user BITBUCKET_PASSWORD=app_pass BITBUCKET_WORKSPACE=workspace ${0##*/} --dry-run
    BITBUCKET_USERNAME=user BITBUCKET_PASSWORD=app_pass BITBUCKET_WORKSPACE=workspace ${0##*/}
    
    # Using Email + Access Token
    BITBUCKET_EMAIL=user@example.com BITBUCKET_ACCESS_TOKEN=token BITBUCKET_WORKSPACE=workspace ${0##*/} --dry-run
    BITBUCKET_EMAIL=user@example.com BITBUCKET_ACCESS_TOKEN=token BITBUCKET_WORKSPACE=workspace ${0##*/}
"
}
...
bitbucket_cleanup() {
    ...
    if [[ -n "$BITBUCKET_USERNAME" && -n "$BITBUCKET_PASSWORD" ]]; then
        ...
        echo "  - BITBUCKET_USERNAME and BITBUCKET_PASSWORD (for app password auth)"
    ...
    if echo "$repos" | jq -e '.error' >/dev/null 2>&1; then
        echo "Error fetching repositories: $(echo "$repos" | jq -r '.error.message')"
        return 1
    fi
    
    # Check if we got a valid response with values array
    if ! echo "$repos" | jq -e '.values' >/dev/null 2>&1; then
        echo "Error: Invalid response format from Bitbucket API"
        echo "Response: $repos"
        return 1
    fi
